...
function setupTsnePlot() {
  ...
  if (!window.Plotly) {
    ...
    )).join("") + '<div class="tsne-fallback-label">Interactive 3D scatter uses Plotly. If the CDN is unavailable, this static map-scene preview keeps the layout intact.</div>';
    return;
  }
  ...
  const traces = [
    makeCluster("Waymo Motion", "#ef4b34", -1.2, 0.2, 0.4),
    makeCluster("nuPlan", "#35a7ff", 0.8, 0.8, -0.2),
    makeCluster("Argoverse 2", "#22c55e", 0.2, -1.0, 0.5),
    makeCluster("OpenLane-V2", "#f59e0b", 1.35, -0.45, -0.5)
  ];
...
function setupMetricsDashboard(horizontalMetrics) {
  ...
  const datasets = horizontalMetrics || {
    success: [
      ["TopoNet", 32.3, "#64748b", "#94a3b8", "rgba(100, 116, 139, 0.13)"],
      ["TopoPoint", 34.0, "#7c3aed", "#a78bfa", "rgba(124, 58, 237, 0.12)"],
      ["TopoGPT", 42.6, "#ef4b34", "#f59e0b", "rgba(239, 75, 52, 0.13)"]
    ],
    generalization: [
      ["TopoNet", 26.3, "#0ea5e9", "#35a7ff", "rgba(14, 165, 233, 0.13)"],
      ["TopoMLP", 28.3, "#16a34a", "#22c55e", "rgba(34, 197, 94, 0.13)"],
      ["TopoGPT", 39.2, "#f97316", "#facc15", "rgba(249, 115, 22, 0.13)"]
    ],
    efficiency: [
      ["Scratch", 7.0, "#8b5cf6", "#d946ef", "rgba(139, 92, 246, 0.13)"],
      ["Frozen prior", 42.1, "#06b6d4", "#2dd4bf", "rgba(6, 182, 212, 0.13)"],
      ["LoRA prior", 42.6, "#ef4b34", "#fb7185", "rgba(239, 75, 52, 0.13)"]
    ]
  };
